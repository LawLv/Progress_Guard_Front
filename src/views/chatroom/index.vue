...
export default {
  ...
  created() {
    ...
    const url = `http://3.88.162.233:8080/user-group/getGroups/`+ sessionStorage.getItem('userId') // 在URL中添加占位符
    axios.get(url)
      .then(response => {
  ...
  methods: {
    selectGroup(groupId) {
      ...
      axios.get('http://3.88.162.233:8080/user-group/getUsers/' + this.selectedGroupId)
        .then(response => {
    ...
    initWebSocket() { // 初始化websocket
      ...
      url = 'ws://3.88.162.233:8080/ws/' + this.selectedGroupId + '/' + sessionStorage.getItem('userId')
